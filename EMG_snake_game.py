...
class SnakeGame:
    ...
    def handle_emg(self):
        try:
            ...
            # No EMG data available, continue with game
            pass
    ...
    def move_snake(self):    
        if not self.previous_key_presses:
            return
        ...
        for i in range(0, len(self.snake_body)):
            if len(self.previous_key_presses) > (1 + i):
    ...
    def run_game(self):
        while self.running: 
            ...
            # Draw Snake
            pygame.draw.rect(self.window, self.head_blue, snake)
            for b in self.snake_body:
                pygame.draw.rect(self.window, self.snake_green, [b[0], b[1], 20, 20])
